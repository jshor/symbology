...
import {SymbologyType} from "../../index"
...
describe("Binary methods", () => {
  ...
  describe("createBuffer()", () => {
    beforeEach(() => {
      jest
        ...
        .mockReturnValue({
          message: "Barcode created", code: 2, width: 32, height: 32, bitmap: [], encodedData: ""
        })
    })
    ...
    it("should call createStream() with the arguments provided by the config", () => {
      ...
      expect(binding.createStream).toHaveBeenCalledWith(barcodeData, symbol.symbology, symbol.height, symbol.whitespaceWidth, symbol.borderWidth, symbol.outputOptions, symbol.backgroundColor, symbol.foregroundColor, symbol.fileName, symbol.scale, symbol.option1, symbol.option2, symbol.option3, 0, "example text", symbol.encoding, symbol.eci, symbol.primary, symbol.rotation, symbol.dotSize)
    })
    ...
    it("should fallback to default values for values missing in config", () => {
      ...
      expect(binding.createStream).toHaveBeenCalledWith(barcodeData, symbol.symbology, symbol.height, symbol.whitespaceWidth, symbol.borderWidth, symbol.outputOptions, symbol.backgroundColor, symbol.foregroundColor, symbol.fileName, symbol.scale, symbol.option1, symbol.option2, symbol.option3, 1, barcodeData, symbol.encoding, symbol.eci, symbol.primary, symbol.rotation, symbol.dotSize)
    })
  })
  ...
  describe("invoke()", () => {
    describe("when symbol creation succeeds without warnings", () => {
      const result = {
        code: 0, message: "Success", bitmap: [], encodedData: "<encoded data>", width: 10, height: 15
      }
      ...
      it("should resolve with the resulting binary data when symbology is successfully rendered", async () => {
        ...
        await expect(binary.invoke({
          ...
          ...result, message: "Symbology successfully created."
        })
      })

      it("should apply bitmap output option (8) to existing output options for non-PNG images", async () => {
        const config = {
          symbology: SymbologyType.CODE128, outputOptions: OutputOption.BARCODE_BIND
        }
      ...
      it("should set the output options to bitmap output (8) to non-PNG images wth no previous output specified", async () => {
        ...
        expect(binary.createBuffer).toHaveBeenCalledTimes(1)
        expect(binary.createBuffer).toHaveBeenCalledWith({
          ...config, fileName: "out.svg", outputOptions: 8
        }, barcodeData)
      })

      it("should not mutate the output options for PNG images", async () => {
        const config = {
          symbology: 10, outputOptions: OutputOption.BARCODE_BIND
        }
        const barcodeData = "12345"
    ...
    describe("when symbol creation succeeds with a warning", () => {
      it("should resolve if the status code is greater than 2", async () => {
        const result = {
          code: 1, message: "Created with warnings", bitmap: [], encodedData: "<encoded data>", width: 10, height: 15
        }
    ...
    describe("when the symbol creation fails", () => {
      it("should reject if the status code is greater than 2", async () => {
        const result = {
          code: 3, message: "Failure", bitmap: [], encodedData: "<encoded data>", width: 10, height: 15
        }
